...
<script>
  ...
  export default {
    ...
    async created() {
      ...
      // this.editButtonIsVisible = (this.article.authorId == this.$store.state.id)
    },
    watch: {
      '$route': 'getArticle'
    },
    ...
    computed: {
      editButtonIsVisible() {
        return (this.article.authorId == this.$store.state.id)
      },
      authorid() {
    ...
    methods: {
      ...
      async getArticle() {
        let that = this
        await viewArticle(that.$route.params.id).then(data => {
      ...
      getCommentsByArticle() {
        ...
        getCommentsByArticle(that.article.id).then(data => {
          if(data.success){
            that.article.commentCounts = data.data.length
